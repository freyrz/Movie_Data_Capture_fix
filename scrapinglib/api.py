...
class Scraping:
            ...
            try:
                ...
                try:
                    ...
                    if data == 404:
                        continue
                    if len(json_data) > 2:
                        data = self.cover_data(data, json_data)
                    if type(data) == str:
                        json_data = json.loads(data)
                    else:
                        json_data = data
    ...
    def cover_data(self, new_data, old_data):

        # print(new_data)
        if len(new_data) == 2:
            return old_data
        new_data = json.loads(new_data)
        # print(old_data["actor"])
        if len(old_data["actor"]) == 0:
            old_data["actor"] = new_data["actor"]
        if len(old_data["actor_photo"]) == 0:
            old_data["actor_photo"] = new_data["actor_photo"]
        if len(old_data["cover"]) == 0:
            old_data["cover"] = new_data["cover"]
        if len(old_data["cover_small"]) == 0:
            old_data["cover_small"] = new_data["cover_small"]
        if len(old_data["director"]) == 0:
            old_data["director"] = new_data["director"]
        if len(old_data["extrafanart"]) == 0:
            old_data["extrafanart"] = new_data["extrafanart"]
        if len(old_data["number"]) == 0:
            old_data["number"] = new_data["number"]
        if len(old_data["outline"]) == 0:
            old_data["outline"] = new_data["outline"]
        if len(old_data["release"]) == 0:
            old_data["release"] = new_data["release"]
        if len(old_data["runtime"]) == 0:
            old_data["runtime"] = new_data["runtime"]
        if len(old_data["series"]) == 0:
            old_data["series"] = new_data["series"]
        if len(old_data["studio"]) == 0:
            old_data["studio"] = new_data["studio"]
        if len(old_data["tag"]) == 0:
            old_data["tag"] = new_data["tag"]
        if len(old_data["title"]) == 0:
            old_data["title"] = new_data["title"]
        if len(old_data["year"]) == 0:
            old_data["year"] = new_data["year"]
        if len(old_data["website"]) == 0:
            old_data["website"] = new_data["website"]
